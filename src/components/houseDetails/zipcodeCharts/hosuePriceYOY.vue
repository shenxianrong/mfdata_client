    ...
    <div class="echart-chart">
        <div class="inside">
            ...
            <div class="chartBox clearfix">
                ...
                <div class="chart-div" id="housePriceYOY-chart" v-show="!chartNoDataShow" style="height: 85%; top: 55%;"></div>
                <h4 ref="housePriceYOY_chart_NoData" v-show="chartNoDataShow"></h4>
            </div>
...
<script>
    export default {
        ...
        mounted() {
            this.myEcharts = this.$echarts.init(document.getElementById('housePriceYOY-chart'));
        ...
        methods: {
            //获取本区域房价同比增长率数据
            get_housePriceYOY() {
                this.$get('/report/yoy/' + this.$store.getters.get_houseDetailsZipcode +'?time='+ this.filterYear).then(res => {
                    const data = res;
                    if (data.status == 0) {
                        this.chartNoDataShow = false;
                        this.housePriceYOY_charts(data.data);
                    } else {
                        this.chartNoDataShow = true;
                        this.$refs.housePriceYOY_chart_NoData.innerText = data.msg;
                    }
                });
            },
            //本区域房价同比增长率——折线图
            housePriceYOY_charts(data){
