<template>
    <section>
        <diV v-if="orderManagement.length">
            <Table :columns="columns" :data="orderManagement"></Table>
            <br />
        ...
        <!--未成功支付的订单提交支付弹出框-->
        <Modal v-model="modalContinue" width="360">
            <p slot="header" style="color:rgb(99, 95, 95);text-align:center">
                <Icon type="home"></Icon>
                <span>优房数据提示您：</span>
            </p>
            <div style="text-align:left; line-height: 36px;">
                <p>订单编号：{{orderData.orderno}}</p>
                <p>订单名称：{{orderData.name}}</p>
...
<script>
  export default {
      ...
      return {
        columns: [
          ...
          },
          {
            ...
            key: 'operation',
            align: 'center',
            render: (h, params) => (this.statusReturnBtn (h, params))
          }
        ],  // 订单管理表头
        ...
        modalContinue: false, // 未成功支付的订单提交支付  弹出框
        orderData:{} // 未支付订单数据
      }
    },
    computed: {},
    ...
    methods: {
      // 获取订单管理数据列表
      getOrderManagement () {
        this.$get('/orderList').then(res => { // 换接口
          let data = res.data
          ...
          if (res.code === 0) {
            ...
            data.forEach((item, index) => {
              this.arrayData.push({
                name: `${item.amount}元${item.reportNumber}份`,
                ctime: this.getDateDiff(item.ctime),
                orderno: item.orderno,
                amount: `¥${item.amount}`,
                status: this.statusOrder(item.status),
                operation:item.status
              })
            })
            // 不够十条数据全部展示、超过十条先展示十条
            if ( data.length < this.pageSize) {
              this.orderManagement = this.arrayData
            } else {
              this.orderManagement = this.arrayData.slice(0,this.pageSize)
            }
          }else{
      ...
      createCode () {
        ...
        this.modalCode = true
        // 请求接口
       // console.log(this.selectCount) //选择的份数
      ...
      changePage (index) {
        ...
        this.orderManagement = this.arrayData.slice(start, end)
      },
      // 提交支付
      submitPay (index) {
        this.modalContinue = true
        this.orderData = {
          name: this.orderManagement[index].name, //订单名称
          orderno: this.orderManagement[index].orderno, // 订单编号
          amount: this.orderManagement[index].amount // 订单金额
        }
      },
      // 再次购买
      againPay (index) {
      ...
      // 判断订单状态返回不同的操作按钮
      statusReturnBtn (h, params) {
        let data = this.orderManagement[params.index].operation
